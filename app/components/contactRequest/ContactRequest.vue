  ...
  <div class="bg-primary flex justify-center items-center min-h-screen py-10">
    <div class="w-full max-w-md p-8 bg-white rounded shadow-lg">
      <div class="text-center mb-6">
        ...
        />
        <h2 class="text-2xl font-bold mt-4">Quero conhecer o Clinux!</h2>
        <p class="text-sm mt-2">Preencha seus dados abaixo e entraremos em contato</p>
      </div>

      <form @submit.prevent="submitForm">
        <!-- Nome -->
        <UFormGroup label="Nome*" :error="!name && 'Campo obrigatório'">
          <UInput v-model="name" placeholder="Digite seu nome" />
        </UFormGroup>

        <!-- Email -->
        <UFormGroup label="Email*" :error="!email && 'Campo obrigatório'">
          <UInput v-model="email" type="email" placeholder="Digite seu email" />
        </UFormGroup>

        <!-- Telefone -->
        <UFormGroup label="Telefone*" :error="!phone && 'Campo obrigatório'">
          <UInput v-model="phone" type="tel" placeholder="Digite seu telefone" />
        </UFormGroup>

        <!-- Empresa -->
        <UFormGroup label="Empresa*" :error="!company && 'Campo obrigatório'">
